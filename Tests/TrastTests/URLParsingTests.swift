...
enum URLParsingTests {
    private static func parse(_ string: String) -> TrastURL.Action? {
        guard let url = URL(string: string) else { return nil }
        return TrastURL.parse(url)
    }

    static func runAll(_ t: TestRunner) {
        t.run("URLParsing.launcher") {
            t.check(parse("trast://launcher") == .launcher, "launcher not parsed")
            t.check(parse("trast://picker") == .launcher, "legacy picker should still work")
        }

        t.run("URLParsing.applyByName") {
            t.check(parse("trast://apply?name=Left%20Half") == .apply(name: "Left Half"), "apply not parsed")
        }

        t.run("URLParsing.applyWithoutNameIsRejected") {
            t.check(parse("trast://apply") == nil, "apply without name should be rejected")
        }

        t.run("URLParsing.wrongSchemeIsRejected") {
            t.check(parse("https://trast/launcher") == nil, "https should be rejected")
            t.check(parse("raycast://launcher") == nil, "raycast scheme should be rejected")
        }

        t.run("URLParsing.unknownHostIsRejected") {
            t.check(parse("trast://bogus") == nil, "unknown host should be rejected")
        }

        t.run("URLParsing.ephemeralRelativeSizeAndPosition") {
            guard case .ephemeral(let command)? = parse("trast://command?position=center&relativeWidth=0.5&relativeHeight=0.5") else {
        ...
        t.run("URLParsing.ephemeralAbsoluteSizeAndOffsets") {
            guard case .ephemeral(let command)? = parse("trast://command?absoluteWidth=500&relativeXOffset=-0.1&absoluteYOffset=40") else {
                t.check(false, "expected ephemeral action")
                return
            }
        ...
        t.run("URLParsing.ephemeralDefaultsToTopLeftAnchor") {
            guard case .ephemeral(let command)? = parse("trast://command?relativeWidth=1") else {
                t.check(false, "expected ephemeral action")
                return
            }
