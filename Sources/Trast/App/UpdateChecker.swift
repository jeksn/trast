...
enum UpdateChecker {
    private static let repo = "jeksn/trast"
    ...
    private static func presentUpdateIfAvailable(_ release: Release?) {
        ...
        let alert = NSAlert()
        alert.messageText = "Trast \(release.version) is available"
    ...
    private static func present(_ release: Release?) {
        ...
        guard VersionCompare.isNewer(release.version, than: currentVersion) else {
            showAlert(
                ...
                message: "Trast \(currentVersion) is the latest version."
            )
            return
        }
        ...
        Task {
            do {
                ...
                let dmgDestination = FileManager.default.temporaryDirectory
                    .appendingPathComponent("Trast-\(version).dmg")
    ...
    private static func install(dmg: URL) throws {
        ...
        let source = URL(fileURLWithPath: mountPoint).appendingPathComponent("Trast.app")
        let destination = URL(fileURLWithPath: "/Applications/Trast.app")
    ...
    private static func relaunch() {
        ...
        process.arguments = ["-c", "sleep 1; pkill -x Trast; sleep 0.5; open /Applications/Trast.app"]
        try? process.run()
        NSApp.terminate(nil)
    }
